...
import { createStacksRoutes } from './routes/stacks'
...
try {
  ...
  const openCodeAdapter = new OpenCodeAdapter(openCodeClient, openCodeSupervisor)
  agentRegistry.register(openCodeAdapter)

  const openClaudeConfig: Record<string, string> = {}
  if (ENV.OPENCLAUDE?.COMMAND) openClaudeConfig.command = ENV.OPENCLAUDE.COMMAND
  if (ENV.OPENCLAUDE?.CONFIG_DIR) openClaudeConfig.configDir = ENV.OPENCLAUDE.CONFIG_DIR
  if (ENV.OPENCLAUDE?.WORKING_DIR) openClaudeConfig.workingDir = ENV.OPENCLAUDE.WORKING_DIR
  const openClaudeAdapter = new OpenClaudeAdapter(openClaudeConfig)
  agentRegistry.register(openClaudeAdapter)
  openClaudeAdapter.start().catch((err) => {
...
protectedApi.route('/plugins', createPluginsRoutes(db, openCodeClient))
...
protectedApi.route('/stacks', createStacksRoutes())
...
const shutdown = async (signal: string) => {
  ...
  try {
    ...
    const agentRegistry = getAgentRegistry()
    for (const adapter of agentRegistry.list()) {
      try {
        await adapter.stop()
        logger.info(`Agent adapter '${adapter.id}' stopped`)
      } catch (error) {
        logger.warn(`Failed to stop agent adapter '${adapter.id}':`, error)
      }
    }
