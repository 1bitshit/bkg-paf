...
type SessionV2InfoRaw = SessionV2InfoV1 | SessionV2InfoV2
...
function isNewSession(session: SessionV2InfoRaw): session is SessionV2InfoV2 {
  return 'location' in session && session.location !== undefined
}

function toLegacySession(session: SessionV2InfoRaw, directory?: string): LegacySession {
...
export type SessionV2Info = SessionV2InfoV2
...
export class OpenCodeClient {
  ...
  async listSessionsV2(params?: SessionPageParams): Promise<{ items: SessionV2Info[]; nextCursor?: string }> {
    const response = await fetchWrapper<SessionPageResponse>(`${this.baseURL}/api/session`, {
    ...
    const items = (response.data ?? []).filter(isNewSession)
    return {
      items,
      nextCursor: response.cursor?.next,
    }
