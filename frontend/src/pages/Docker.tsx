...
function StackStatusBadge({ status }: { status: StackInfo['status'] }) {
  if (status === 'running') return <Badge className="bg-emerald-500/15 text-emerald-400 border-emerald-500/20"><CheckCircle2 className="w-3 h-3 mr-1" />Running</Badge>
  if (status === 'stopped') return <Badge className="bg-red-500/15 text-red-400 border-red-500/20"><XCircle className="w-3 h-3 mr-1" />Stopped</Badge>
  if (status === 'error') return <Badge className="bg-orange-500/15 text-orange-400 border-orange-500/20"><AlertTriangle className="w-3 h-3 mr-1" />Error</Badge>
  return <Badge variant="outline"><Loader2 className="w-3 h-3 mr-1" />Unknown</Badge>
}
      ...
      <Card className="w-full max-w-4xl max-h-[80vh] flex flex-col">
        ...
        <div className="flex-1 overflow-auto p-4">
          {loading ? (
            ...
            <pre ref={logsRef} className="text-xs font-mono whitespace-pre-wrap text-muted-foreground bg-muted/30 rounded p-4 max-h-[60vh] overflow-auto">{logs}</pre>
          )}
        </div>
      </Card>
    </div>
...
function StackCard({ stack, onSelect }: { stack: StackInfo; onSelect: (name: string) => void }) {
  return (
    <Card className="group cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => onSelect(stack.name)}>
      <CardContent className="p-4">
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
            ...
            {stack.composeFile && (
              <div className="text-xs text-muted-foreground">{stack.composeFile}</div>
            )}
        ...
        <div className="flex-1 overflow-auto p-4 space-y-4">
          <div>
            ...
            <Input
              ...
              pattern="[a-z0-9_-]+"
            />
            <div className="text-xs text-muted-foreground mt-1">Only lowercase letters, numbers, underscores, and hyphens</div>
          </div>
          ...
          <div>
            <label className="text-sm font-medium mb-1 block">.env (optional)</label>
            <Textarea
              value={envContent}
...
function StackDetail({ stackName, onBack }: { stackName: string; onBack: () => void }) {
  ...
  const handleDelete = async () => {
    if (!confirm(`Delete stack "${stackName}"? This will stop all services and remove the compose files.`)) return
    setOperating('delete')
      ...
      <div className="flex-1 overflow-auto p-4 space-y-4">
        <div className="flex flex-wrap gap-2">
          {stack.status === 'stopped' || stack.status === 'unknown' ? (
            ...
            >
              {operating === 'start' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Play className="w-4 h-4 mr-1" />}
              Start
            </Button>
          ) : (
            <Button
            ...
            >
              {operating === 'stop' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Square className="w-4 h-4 mr-1" />}
              Stop
            </Button>
          )}
          ...
          <Button
            ...
            disabled={operating !== null}
          >
            {operating === 'restart' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <RotateCw className="w-4 h-4 mr-1" />}
            Restart
          </Button>
          ...
          <Button
            ...
            disabled={operating !== null}
          >
            {operating === 'down' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Square className="w-4 h-4 mr-1" />}
            Down
          </Button>
          ...
          <Button
            ...
            disabled={operating !== null}
          >
            {operating === 'pull' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Download className="w-4 h-4 mr-1" />}
            Pull
          </Button>
          ...
          <Button
            ...
            disabled={operating !== null}
          >
            {operating === 'update' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <RefreshCw className="w-4 h-4 mr-1" />}
            Update
          </Button>
          ...
          >
            {operating === 'deploy' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Upload className="w-4 h-4 mr-1" />}
            Deploy
          </Button>
          ...
          >
            {operating === 'delete' ? <Loader2 className="w-4 h-4 animate-spin mr-1" /> : <Trash2 className="w-4 h-4 mr-1" />}
            Delete
          </Button>
        </div>
                ...
                {stack.services.map((svc) => (
                  <div key={svc.name} className="flex items-center justify-between p-2 rounded bg-muted/30">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={svc.status === 'running' ? 'default' : 'outline'} className="text-xs">
                        {svc.status}
                      </Badge>
                      {svc.health && (
                        <Badge variant={svc.health === 'healthy' ? 'default' : 'destructive'} className="text-xs">
                          {svc.health}
                        </Badge>
                      )}
                    </div>
        ...
        <Card>
          <CardContent className="p-4">
            ...
            />
            {validation && (
              <div className={`mt-2 text-xs p-2 rounded ${validation.valid ? 'text-emerald-400 bg-emerald-500/10' : 'text-red-400 bg-red-500/10'}`}>
                {validation.valid ? '✅ YAML is valid' : `❌ ${validation.error || 'Invalid YAML'}`}
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-medium flex items-center gap-2">
                <Settings className="w-4 h-4" />
                .env
                {dirty && envContent !== (stack?.envContent || '') && <Badge variant="outline" className="text-xs">Modified</Badge>}
              </h3>
              <div className="flex items-center gap-2">
                <Button
                  variant="ghost"
...
export function Docker() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true)
    await Promise.all([fetchContainers(), fetchImages(), fetchInfo(), fetchStacks()])
    setLoading(false)
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as DockerTab)}>
          ...
          <TabsContent value="stacks" className="space-y-2 mt-3">
            ...
            {loading && stacks.length === 0 ? (
              ...
              <Card><CardContent className="p-8 text-center text-muted-foreground">No stacks found. Create one to get started.</CardContent></Card>
            ) : (
              stacks.map((stack) => (
                <StackCard key={stack.name} stack={stack} onSelect={setSelectedStack} />
              ))
            )}
          </TabsContent>
          ...
          <TabsContent value="containers" className="space-y-2 mt-3">
            {loading && containers.length === 0 ? (
              ...
              containers.map((c) => (
                <ContainerCard key={c.id} container={c} onAction={handleContainerAction} />
              ))
            )}
          </TabsContent>
